...
1. **Determine which browser the user is running before anything else.** This is the mandatory first step. If the user hasn't already specified their browser, ask. Common browsers: `chrome`, `chrome-beta`, `edge`, `bitbrowser`. If multiple browsers are installed with the extension, use `open-browser-use profiles --connected` to list available targets.
2. Once the browser is known, pass `--browser <selector>` to every CLI command for the rest of the task. Verify the connection with `open-browser-use ping --browser <browser>`. If ping fails, ask the user to open that browser and ensure the extension is enabled. If the extension is not installed, guide the user through setup ([references/installation.md](references/installation.md)).
