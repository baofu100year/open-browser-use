...
1. **Determine which browser and profile are connected before anything else.** This is the mandatory first step. Run `open-browser-use profiles --connected`. It outputs BROWSER, DIRECTORY, DISPLAY NAME, and CONNECTED for each browser/profile that has the extension installed.
   - If nothing is connected: ask the user to open their browser, then retry. If still nothing, guide the user through setup ([references/installation.md](references/installation.md)).
   - If exactly one profile is connected: use that browser and profile for all subsequent commands.
   - If multiple profiles are connected: ask the user which one to use, then lock in that choice.
2. Once a browser/profile is chosen, pass `--browser <selector>` and `--profile <selector>` to every CLI command for the rest of the task. Verify with `open-browser-use ping --browser <browser> --profile <profile>`. See "Browser and profile handling" below for full selection rules.
...
- At the start of a related follow-up task, list all user tabs and check `tabGroup`, `title`, and `url` before creating anything new. Claim an existing matching deliverable or handoff tab — or a local dev server page the user already has open — into the current session; only open a new tab when no suitable tab exists.
